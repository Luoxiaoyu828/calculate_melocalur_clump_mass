...
def get_outcat_all():
    R2_data_params_path = r'R2_data_params'
    file_list = [os.path.join(R2_data_params_path, item + '\MWISP_outcat.csv') for item in
                 os.listdir(R2_data_params_path)]
    ...
    outcat_all.to_csv('outcat_all_d.csv', sep='\t', index=False)
...
def plot_item(data, fig_name=None, xlabel=r'Tex $[K]$', rect=None, bins=25, uint='K', xlims=None, fitting=True):
    """
    绘制参数的统计直方图
    """
...
if __name__ == '__main__':
    # get_outcat_all()
    outcat_all1 = pd.read_csv('outcat_all.csv', sep='\t')
    d_min = 1.5
    d_max = 2.5
    ...
    save_path = 'Distance_%.1fto%.1f_1' %(d_min, d_max)
    ...
    plot_item(tex_max, xlabel=r'Tex max $[K]$', fig_name=os.path.join(save_path, 'tex_max.png'), bins=50)
    plot_item(tex_mean, xlabel=r'Tex mean $[K]$', fig_name=os.path.join(save_path, 'tex_mean.png'), bins=50)

    tao_mean = outcat_all['tao_max(-)'].values
    plot_item(tao_mean, xlabel=r'$\tau$', rect=[0.1, 0.1, 0.8, 0.88], uint='', bins=50, xlims=[0, 2],
              fig_name=os.path.join(save_path, 'tao_mean.png'))

    v_fwhm = outcat_all['v_fwhm(km/s)'].values
    plot_item(v_fwhm, xlabel=r'$\Delta v_{FWHM}$ [km s$^{-1}$]', uint='km s$^{-1}$', rect=[0.08, 0.11, 0.85, 0.87],
              fig_name=os.path.join(save_path, 'v_fwhm.png'))
    nh2_mean = outcat_all['nh2_mean(10**21*cm-2)'].values
    plot_item(nh2_mean, xlabel=r'N(H$_2$) [$\times$10$^{21}$ cm$^{-2}$]', uint=r'$\times$10$^{21}$ cm$^{-2}$',
    ...
    plot_item(nh2_max, xlabel=r'N(H$_2$) [$\times$10$^{21}$ cm$^{-2}$]', uint=r'$\times$10$^{21}$ cm$^{-2}$',
              rect=[0.09, 0.11, 0.84, 0.87], bins=200, xlims=[0, 7],
    ...
    plot_item(reff, xlabel=r'Reff [pc]', uint=r'pc', rect=[0.09, 0.11, 0.84, 0.87], bins=50, xlims=[0, 10],
              fig_name=os.path.join(save_path, 'reff.png'))

    M_sum = outcat_all['M_sum(Msun)'].values
    M_sum[np.isnan(M_sum)] = 0
    plot_item(np.log10(M_sum), xlabel=r'$M_{LTE}$ $[M_{\odot}]$', uint=r'$M_{\odot}$', rect=[0.09, 0.11, 0.84, 0.87],
              bins=50, fig_name=os.path.join(save_path, 'M_lte.png')
              )

    M_vir = outcat_all['M_vir(Msun)'].values
    plot_item(np.log10(M_vir), xlabel=r'$M_{vir}$ $[M_{\odot}]$', uint=r'$M_{\odot}$', rect=[0.09, 0.11, 0.84, 0.87],
              bins=50, fig_name=os.path.join(save_path, 'M_vir.png'))

    vir_a = outcat_all['vir_a(-)'].values
    vir_a[np.isnan(vir_a)] = 0
    plot_item(vir_a, xlabel=r'vir_a [-]', uint=r'-', rect=[0.09, 0.11, 0.84, 0.87], bins=100,
              xlims=[0, 10], fig_name=os.path.join(save_path, 'vir_a.png'), fitting=False)

    Velocity = outcat_all['Velocity'].values
    plot_item(Velocity, xlabel=r'Velocity', uint=r'km s$^{-1}$', rect=[0.09, 0.11, 0.84, 0.87], bins=50,
             fitting=False, fig_name=os.path.join(save_path, 'Velocity.png'))

    Distance = outcat_all['Distance(kpc)'].values
    plot_item(Distance, xlabel=r'Distance [kpc]', uint=r'kpc', rect=[0.09, 0.11, 0.84, 0.87], bins=30,
    ...
    plot_item(z, xlabel=r'z [pc]', uint=r'pc', rect=[0.09, 0.11, 0.84, 0.87], bins=50, fitting=False,
