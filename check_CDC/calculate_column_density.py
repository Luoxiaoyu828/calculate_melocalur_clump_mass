...
class CO_Data:
    def __init__(self, co_12_path=None, co_13_path=None, co_18_path=None):
        self.co_13_path = co_13_path    # 'data/original_data/hdu0_mosaic_L.fits'
        self.co_12_path = co_12_path    # 'data/original_data/hdu0_mosaic_U.fits'
        self.co_18_path = co_18_path    # 'data/original_data/hdu0_mosaic_L2.fits'

        self.m16_13_data, self.wcs13 = self.dd_header(self.co_13_path)
        self.m16_12_data, self.wcs12 = self.dd_header(self.co_12_path)
        self.m16_18_data, self.wcs18 = self.dd_header(self.co_18_path)
...
class Calculate_Parameters:
    ...
    def __init__(self, outcat, pdf_path, co_12_path=None, co_13_path=None, co_18_path=None, co='13'):
        self.pdf_path = pdf_path
        if not os.path.exists(self.pdf_path):
            os.mkdir(self.pdf_path)

        record = pd.read_csv(outcat, sep='\t')
        self.core_num = record.shape[0]
        self.co = co

        self.co_data = CO_Data(co_12_path=co_12_path, co_13_path=co_13_path, co_18_path=co_18_path)
    ...
    def get_data_cube(self):
        if self.co == '13':
            ...
            data = self.co_data.m16_13_data
        else:
            wcs = self.co_data.wcs18
            data = self.co_data.m16_18_data
        ...
        data_cube = [data[i[2]:j[2], i[1]:j[1], i[0]:j[0]] for i, j in zip(range_down, range_up)]

        local_wcs = [wcs[i[2]:j[2], i[1]:j[1], i[0]:j[0]] for i, j in zip(range_down, range_up)]
    ...
    def get_data12_cube(self):
        ...
        data = self.co_data.m16_12_data

        # outcat table core center
        cen_pt = self.record[['Galactic_Longitude', 'Galactic_Latitude', 'Velocity']].values

        cen_pt_pix = np.array(wcs.all_world2pix(cen_pt[:, 0], cen_pt[:, 1], cen_pt[:, 2], 0)).T
    ...
    def calculate_tex(self):
        ...
        :return:
            激发温度Tex, tr
        """
        data_list = self.data12
        tex = []
        tr = []
        for data in data_list:
            tr_ = data.max(axis=0) * u.K
            v0 = self.v0_12
            h = self.h
            k = self.k
            T_bg = self.T_bg

            [size_i, size_j] = tr_.shape
            tex_ = np.zeros_like(tr_)
            for i in range(size_i):
                for j in range(size_j):
                    tex_[i, j] = (h*v0/k) / (ln(1 + (h*v0/k) / (tr_[i, j] + planck_function(T_bg, v0))))
            tex.append(tex_)
            tr.append(tr_)
        return tex, tr
    ...
    def calculate_vfwhm(self):
        ...
        for data in data_list:
            ...
            fwhm_ = (8 * ln(2)) ** 0.5 * popt[-1].__abs__() * 0.167 * u.km / u.second
            Tr.append(Tr_)
            fwhm.append(fwhm_)
    ...
    def calculate_tao(self):
        ...
        tex_list = self.tex
        tao = []
        for data, tex in zip(data_list, tex_list):
            tr = data.max(axis=0) * u.K

            tao_ = np.zeros_like(tex.value)
            [size_i, size_j] = tao_.shape
            for i in range(size_i):
                for j in range(size_j):
                    try:
                        tao_[i, j] = - ln(1 - tr[i, j] / (planck_function(tex[i, j], v0) - planck_function(T_bg, v0)))
                    except ValueError:
                        tao_[i, j] = 0
                        print(1)
            tao.append(tao_)
